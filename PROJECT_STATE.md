...
- **고객 플랫폼** `/customer`, `/customer/equipment`, `/customer/requests`, `/customer/services`
  ...
  - **햄버거 드로어** — 계정·고객사 전환 / 홈·내 장비·요청 내역 / 서비스 / 화면 전환
...
- **서비스 허브** `/customer/services` — 단계별 색 밴드에 카드 그리드, 카드 → 상세 시트,
  각 항목마다 [준비된 것 / 구현 방식 / 고객 가치 / 수익 구조 / 선행 조건], 관심 표시
...
- 고객 플랫폼은 데모 고객사 3곳(한성정밀화학·대한환경기술원·서진플랜트) 중 하나로 보며, 전환은 설정(`customer`)에 저장된다. 실제 인증이 붙으면 `use-customer.ts` 구현부만 세션 기반으로 바꾼다.
