...
| `/customer/services` | **서비스 & 확장 로드맵** — 이용 가능 3 / 준비 중 4 / 검토 중 4 |

두 Surface는 양쪽 상단의 **[관리자 화면 | 고객 화면] 스위처**로 오갑니다 (시연용).
두 칸이 항상 함께 보이고 현재 있는 쪽이 채워져 있어, 지금 위치와 이동 방향이 같이 읽힙니다.

고객 플랫폼 우상단의 **햄버거 버튼**은 [계정 / 메뉴 / **이 페이지 목차** / 화면 전환]을 엽니다.
목차는 페이지마다 다릅니다 — 홈은 섹션, 내 장비는 장비별, 서비스는 3단계 그룹으로 바로 이동합니다.
