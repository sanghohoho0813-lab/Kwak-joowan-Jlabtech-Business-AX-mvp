...
export function LiveClock() {
  ...
  return (
    <div
      className="hidden h-9 shrink-0 items-center gap-2 rounded-xl border border-line bg-ivory-50 px-3 text-xs font-medium text-inkbody lg:flex"
      aria-label="현재 날짜와 시각"
    >
      <CalendarDays className="h-3.5 w-3.5 shrink-0 text-pine-700" />
      {/* 좁은 화면에서는 시각만 노출해 탑바가 밀리지 않게 한다 */}
      <span className="num hidden whitespace-nowrap xl:inline">{date}</span>
